...
The Maid module provides methods to manage tasks. A **task** is any value that
encapsulates the finalization of some procedure or state. "Cleaning" a task
invokes it, causing the procedure or state to be finalized.

All tasks have the following contract when cleaning:

- A task must not produce an error.
- A task must not yield.
- A task must finalize only once; cleaning an already cleaned task should be a
  no-op.
...
The **wrap** function encapsulates the given tasks in a function that cleans
them when called.
...
- A named task can be individually unassigned from the maid without cleaning it.
...
the maid will still hold on to the value. This might be used to hold off garbage
collection of a value that otherwise has only weak references.

A maid is not reusable; after a maid has been destroyed, any tasks assigned to
the maid are cleaned immediately.
...
The **Assign** method performs an action depending on the type of *task*. If
*task* is nil, then the task assigned as *name* is cleaned, if present.
Otherwise, *task* is assigned to the maid as *name*. If a different task
(according to rawequal) is already assigned as *name*, then it is cleaned.
...
Assigning a task with a name that is already assigned cleans the previous task
first.
...
The **AssignEach** method assigns each given argument as an unnamed task.
...
The **Clean** method receives a number of names, and cleans the task assigned to
the maid for each name. Does nothing if the maid is destroyed, and does nothing
for names that have no assigned task.
...
connection is assigned as an unnamed task instead. Does nothing if the maid is
destroyed.

Connect is the preferred method when using maids to manage signals, primarily to
resolve problems concerning the assignment to a destroyed maid:
- Slightly more efficient than regular assignment, since the connection of the
  signal is never made.
...
The **Destroy** method cleans all tasks currently assigned to the maid. Does
nothing if the maid is destroyed.
...
returning the task. Returns nil if no task is assigned as *name*, or if the maid
is Destroyed.
...
The **Wrap** method encapsulates the maid by returning a function that cleans
the maid when called.
...
Assigns a task according to the [Assign][Maid.Assign] method, where the index is
the name, and the value is the task. If the index is a string that is a single
underscore, then the task is assigned according to [AssignEach][Maid.AssignEach]
instead.
